...
qemu\qemu-system-x86_64.exe -trace usb_host   -m 33 -drive id=disk,file=images\boot.img,if=none -device ide,id=ahci -device ide-drive,drive=disk,bus=ahci.0  -no-reboot -vga std -D aa.txt -monitor stdio -usbdevice mouse
pause