...
qemu\qemu-system-x86_64.exe -trace usb_host  -hdc images\boot.img -m 33 -drive id=disk,file=images\a.img,if=none -device ahci,id=ahci -device ide-drive,drive=disk,bus=ahci.0  -no-reboot -vga std -D aa.txt -monitor stdio
pause