...
qemu\qemu-system-x86_64.exe -trace usb_host  -smp cpus=3,cores=3 -m 50 -drive id=disk,file=images\boot.img,if=none -device ahci,id=ahci -device ide-drive,drive=disk,bus=ahci.0  -no-reboot -vga std -D aa.txt -monitor stdio -usbdevice mouse -net nic
pause